...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if (disableSaves)
  {
    DoMenu(_9_, NULL, "SAVES ARE DISABLED", "OK", NULL, NULL, NULL, 0, 0, 0);
    STShowCursor(HCiii);
  ...
  memset(gb1A2,0,512);
  SubstDiskLetter((char *)LOCAL_56, "PUT THE GAME SAVE DISK IN ~", 2);
  SubstDiskLetter((char *)LOCAL_106, "THERE IS NO DISK IN ~!", 2);
  do
  {
    DoMenu(_1_, A3,
           TranslateLanguage("READY TO SAVE GAME"),       // "Put the Game Save Disk in ~"
           TranslateLanguage("SAVE AND PLAY"),
           TranslateLanguage("SAVE AND QUIT"),
           //"FORMAT FLOPPY",
           TranslateLanguage("CANCEL"), 0, 0, 0, 0);
    STShowCursor(HC45);
...
//    };
  ...
  DoMenu(_7_, NULL, "SAVING GAME . . .", 0,0,0,0,0,0,0); // "Saving Game"
...
//.........................................................
    swapBlock2(&gb2);
    gb1A2->Block2Checksum = GenChecksum((ui8 *)&gb2, gb1A2->Block2Hash, 128/2);
    swapBlock2(&gb2);

    swapITEM16s();
    gb1A2->ITEM16Checksum = GenChecksum((ui8 *)d.Item16, gb1A2->ITEM16Hash, (16 * d.MaxITEM16)/2);
    swapITEM16s();

    swapCharacterData();
    gb1A2->CharacterChecksum = GenChecksum((ui8 *)&d.CH16482, gb1A2->CharacterHash, 3328/2);
    swapCharacterData();

    swapTimers();
    gb1A2->TimersChecksum = GenChecksum((ui8 *)gameTimers.pTimer(0), gb1A2->TimersHash, (sizeof(TIMER) * gameTimers.MaxTimer())/2);
    swapTimers();

    gameTimers.swapTimerQue();
    gb1A2->TimerQueChecksum = GenChecksum((ui8 *)gameTimers.TimerQueue(), gb1A2->TimerQueHash, (2 * gameTimers.MaxTimer())/2);
    gameTimers.swapTimerQue();

    swapBlock1(gb1A2);
    D0W = ScrambleAndWrite((i16 *)gb1A2);
    swapBlock1(gb1A2);
    if (D0W == 0) goto SaveError;

    swapBlock2(&gb2);
    D0W = WriteScrambled((pnt)&gb2, 128, gb1A2->Block2Hash, &LOCAL_2);
    if (D0W == 0) goto SaveError;
    swapBlock2(&gb2);

    swapITEM16s();
    D0W = WriteScrambled((pnt)d.Item16, 16 * d.MaxITEM16, gb1A2->ITEM16Hash, &LOCAL_2);
    if (D0W == 0) goto SaveError;
    swapITEM16s();

    swapCharacterData();
    D0W = WriteScrambled((pnt)d.CH16482, 3328, gb1A2->CharacterHash, &LOCAL_2);
    swapCharacterData();
    if (D0W == 0) goto SaveError;

    swapTimers();
    D0W = WriteScrambled((pnt)gameTimers.pTimer(0), sizeof(TIMER) * gameTimers.MaxTimer(), gb1A2->TimersHash, &LOCAL_2);
    swapTimers();
    if (D0W == 0) goto SaveError;

    gameTimers.swapTimerQue();
    D0W = WriteScrambled((pnt)gameTimers.TimerQueue(), 2 * gameTimers.MaxTimer(), gb1A2->TimerQueHash, &LOCAL_2);
    gameTimers.swapTimerQue();
    if (D0W == 0) goto SaveError;
    ...
    D0W = WriteAndChecksum((ui8 *)d.pLevelDescriptors, pwA3, 16 * d.dungeonDatIndex->NumLevel());
    swapLevelDescriptors(d.dungeonDatIndex->NumLevel());
    if (D0W == 0) goto SaveError;

    swapPointer10454();
    D0W = WriteAndChecksum((ui8 *)d.objectListIndex, pwA3, 2*d.numColumnPointers);
    swapPointer10454();
    if (D0W == 0) goto SaveError;

    swapPRN10464(d.dungeonDatIndex->ObjectListLength());
    D0W = WriteAndChecksum((ui8 *)d.objectList, pwA3, 2*d.dungeonDatIndex->ObjectListLength());
    swapPRN10464(d.dungeonDatIndex->ObjectListLength());
    if (D0W == 0) goto SaveError;

    SwapIndirectTextIndex();
    D0W = WriteAndChecksum((ui8 *)d.indirectTextIndex, pwA3, 4*d.dungeonDatIndex->NumWordsInTextArray());
    SwapIndirectTextIndex();
    if (D0W == 0) goto SaveError;

    d.sizeOfCompressedText = BE32(d.sizeOfCompressedText);
    D0W = WriteAndChecksum((ui8 *)&d.sizeOfCompressedText, pwA3, 4);
    d.sizeOfCompressedText = BE32(d.sizeOfCompressedText);
    if (D0W == 0) goto SaveError;

    D0W = WriteAndChecksum((ui8 *)d.compressedText, pwA3, 2*d.sizeOfCompressedText);
    if (D0W == 0) goto SaveError;
...
//0e462 d0c0                     ADD.W    D0,A0
      if (D1W != 0) {
        db.swap(dbNum);
        D0W = WriteAndChecksum((ui8 *)db.GetCommonAddress(DBTYPE(dbNum),0), pwA3, D1W*D0W);
        db.swap(dbNum);
      } else
        D0W=1;
      if (D0W==0)
...
SaveError:
  CLOSE(d.datafileHandle);
  DoMenu(_8_, "UNABLE TO SAVE GAME!", NULL, "OK", 0,0,0,0,0,0);
  STShowCursor(HC57);
