...
class Dungeon {
	...
	async readLevels() {
		...
		// read header
		this.seed = this.dungeonFile.read16(0x0);
		this.totalMapsSize = this.dungeonFile.read16(0x2);
		this.mapNumber = this.dungeonFile.read8(0x4);
		this.textListSize = this.dungeonFile.read16(0x6); // in words
		let tmp = this.dungeonFile.read16(0x8);
		this.startPos = {
			level: 0,
			x: (tmp & 0x3e0) >> 5,
			y: tmp & 0x1f,
		...
		this.objectListSize = this.dungeonFile.read16(0xa); // in words
		this.objectNb = {
			doors: this.dungeonFile.read16(0xc),
			teleporters: this.dungeonFile.read16(0xe),
			texts: this.dungeonFile.read16(0x10),
			actuators: this.dungeonFile.read16(0x12),
			creatures: this.dungeonFile.read16(0x14),
			weapons: this.dungeonFile.read16(0x16),
			cloths: this.dungeonFile.read16(0x18),
			scrolls: this.dungeonFile.read16(0x1a),
			potions: this.dungeonFile.read16(0x1c),
			containers: this.dungeonFile.read16(0x1e),
			miscellaneous: this.dungeonFile.read16(0x20),
		...
		for (let i=0; i<this.mapNumber; i++) {
			this.dungeonFile.seek(mapsOffset + 16*i);
			let newlevel = {};
			newlevel.dataOffset = this.dungeonFile.read16();
			this.dungeonFile.skip(4);
			newlevel.offsetX = this.dungeonFile.read8();
			newlevel.offsetY = this.dungeonFile.read8();
			tmp = this.dungeonFile.read16();
			newlevel.height = ((tmp & 0xf800) >> 11) + 1;
			newlevel.width = ((tmp & 0x7c0) >> 6) + 1;
			...
			tmp = this.dungeonFile.read16();
			newlevel.difficulty = (tmp & 0xf000) >> 12;
			newlevel.maxCreatures = (tmp & 0xf0) >> 4; // max 15 ?
			newlevel.mbDoorDecoration = tmp & 0xf;
			tmp = this.dungeonFile.read16();
			// 0:Grate door, 1:Wooden door, 2:Iron door, 3:Ra door
			newlevel.doorType1Idx = (tmp & 0xf000) >> 12;
			newlevel.doorType0Idx = (tmp & 0xf00) >> 8;

			this.maps.push(newlevel);
			// console.log(`floor: ${newlevel.floor} width:${newlevel.width}, height:${newlevel.height}, difficulty:${newlevel.difficulty}`);
		}
		// console.log(JSON.stringify(this.maps));

		let columnNb = this.maps.reduce((sum, lvl) => sum + lvl.width, 0);

		// tiles with objects
		// we don't read it but it is a word for each column of each map (can be calculated)

		// object on tiles
		// no read for now
		const itemsOffset = mapsOffset + 16 * this.mapNumber;
		const tilesWithItemsOffset = mapsOffset + 16 * this.mapNumber;
		...
		for (let i=0; i<this.textListSize; i++) {
			let w = this.dungeonFile.read16();
			for (let t of [[0x7c00, 10], [0x3e0, 5], [0x1f, 0]]) {
				let c = (w & t[0]) >> t[1];
				if (c === 31) {
					tmp = tmp.replace(/%(.)/g, (s, p1) => {
						return {
							B: '',
							C: 'THE ',
							D: 'YOU ',
				...
				if (c < 26)
					...
					throw new Error(`Found an 'escape code 1' in a text nb ${this.texts.length+1}: ${tmp}`);
				else
					c = [' ', '.', '|', null, '%'][c-26];
				tmp += c;
			}
	...
	getText(idx) {
		return this.texts.find(it => it.index === idx).string;
	}
	getHero(idx) {
		return this.heroes.find(it => it.index === idx);
	}
}
